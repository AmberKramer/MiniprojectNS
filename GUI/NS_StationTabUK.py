...
def treintijden(invoer):
    ...
    label = Label(Root, text="Search results for : " + invoer, width=70, bg="#ffc917")
...
def eindoutput(regels):
    ...
    for regel in regels:
        ...
        if "Let op" in regel:
            ...
            button = tkinter.Button(Root, image=warning_picture, command=lambda text=splitting[1]: on_click(text))
...
warning = "https://orig00.deviantart.net/f92d/f/2018/306/2/6/opmerkingenknop_by_xxdragonsartxx-dcr02rh.png"
warning_picture = PhotoImage(data=urlopen(warning).read())
...
NSlabel = Label(Root, image=NS_picture, borderwidth=0, height=140, width=340)
NSlabel.place(x=1000, y=30)
searching()
Label(Root, bg="#ffc917", text="Search your trainstation:").place(x=510, y=175)
blue = Label(Root, background="#003082", height=3)
blue.pack(side=BOTTOM, fill="x")

Error = Label(Root, text="Enter a valid trainstation!", font=("", 55, "bold"),
              bg="#ffc917", fg="black")

MainMenu = Button(Root, command=nederlands, text="Back", bg="#003082", fg="white", width=20, height=4, anchor="w",
