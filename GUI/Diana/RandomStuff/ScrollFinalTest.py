...
def Bestemming():
    ...
    try:
        ...
        tes=500
        #for r in regel:
        #    fancy=Label(Root,text=r, borderwidth=2, relief="solid", anchor='w', width = 70, height='3')
        #    fancy.place(x=1,y=tes)
        #    tes+=50
        #    list.append(fancy)
        scrollbar = Scrollbar(Root)
        scrollbar.pack(side=RIGHT, fill=Y)
        listbox = Listbox(Root, yscrollcommand=scrollbar.set, width=90)
        for r in regel:
            print(r)
            listbox.insert(ACTIVE, r)
        #for i in range(1000):
